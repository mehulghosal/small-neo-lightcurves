...
for d in dir_names:
	...
	for ld in lc_dirs :
		...
		for f in lc_files :
			...
			# try :
			jd, flux, flux_err  = np.loadtxt(f , unpack=True, skiprows=0)
			...
			param_one , cov_one = curve_fit ( line, seconds_from_start , mag , sigma=mag_err , absolute_sigma=True )
			mag_minus_line = mag - (line(seconds_from_start , *param_one))
			...
			unbinned = ax_lc.errorbar( seconds_from_start , mag_minus_line , mag_err , fmt='s' , markerfacecolor='red' , markeredgecolor='black' , ecolor='black' , capthick=2 , markersize=7 , capsize=3 , label='Line subtracted' )
			ax_lc.set_ylim([-1,1])
			ax_lc.invert_yaxis()
